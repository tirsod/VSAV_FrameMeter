...
local function get_counter_attack_stick()
    ...
    elseif counter_attack_button == 0xE then
        parsed = '720'
    -- Forward first, matching the menu list. Swapped together with it; the two
    -- are separate lists with nothing keeping them in step, so they have to be
    -- edited as a pair (see the v142 note below).
    ...
    -- THE INDEX AND THE MENU LIST ARE TWO SEPARATE LISTS (v142).
    --
    -- menu.lua's counter_attack_stick is what the user picks from; this chain
    -- is what turns the stored INDEX back into a string, and nothing keeps
    -- them in step. Adding "forward"/"back"/"down" to the menu alone put the
    -- index at 21 and this chain fell through to its default, so the setting
    -- read back as 'none' - the log showed `idx=21 stick='none'` and the
    -- dummy pressed the button with no direction at all.
    --
    -- 0x13 is "Character Specific", which this chain never had a case for
    -- either (guardCancel.lua switches on the guard_action, not on this), so
    -- the plain directions continue from 0x14.
    -- 0x13..0x15 are the super jumps, which sit right after Back Dash Cancel
    -- in the menu list. 0x16 is "Character Specific", which this chain still
    -- has no case for, so the plain directions continue from 0x17.
    elseif counter_attack_button == 0x13 then
        parsed = 'super jump forward'
    elseif counter_attack_button == 0x14 then
        parsed = 'super jump'
    elseif counter_attack_button == 0x15 then
        parsed = 'super jump back'
    elseif counter_attack_button == 0x17 then
        parsed = 'forward'
    elseif counter_attack_button == 0x18 then
        parsed = 'back'
    elseif counter_attack_button == 0x19 then
        parsed = 'down'
    end
...
-- P1 ONLY (v186).
--
-- 0x02760E is the game's own `cmpi.b #$8, D0` on the push block press count,
-- and the routine is shared - A6 selects whose push block is being resolved
-- (0xFF8400 P1, 0xFF8800 P2). Without that test the setting silently applied
-- to BOTH players, so raising it to practise against a harder dummy also made
-- the user's own push blocks harder.
--
-- It is the user's constraint: the dummy's push block presses exactly six
-- times (Guard Action = Push Block, measured 32/32 in v185), so any value here
-- would be inert on that side. What the setting is for is "I must press at
-- least this many times", which is P1.
