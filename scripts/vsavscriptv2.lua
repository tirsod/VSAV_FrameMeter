...
-- P2's column is built from MEMORY, one entry per game tick (v135).
--
-- It used to come from joypad.getdown(2), once per displayed frame. The
-- training tool has not driven the dummy through the pad since v131 - the
-- wake-up motion is written straight to $394 - so the pad shows nothing of it,
-- and a displayed frame is about 1.6 game ticks during a knockdown while every
-- entry of the motion lasts exactly one tick.
--
-- guardCancel.lua's per-tick hook queues one entry for every tick on which
-- $122/$123 changed; this turns each of those into the same row table
-- filterinput() would have produced, so everything downstream is unchanged.
--
--   rows  1=l 2=r 3=u 4=d  (5..8 are the diagonals compositeinput makes)
--         9=LP 10=MP 11=HP 12=LK 13=MK 14=HK
--   $123  bit0/bit1 are the two horizontal bits, bit2 down, bit3 up. Which of
--         bit0/bit1 is LEFT depends on the facing byte $b, the same way
--         inputHistory.lua's read_game_input() resolves it.
--   $122  bit0 LP, bit1 MP, bit2 HP, bit4 LK, bit5 MK, bit6 HK
local memory_button_row = { [0] = 9, [1] = 10, [2] = 11, [4] = 12, [5] = 13, [6] = 14 }

local function memoryinput(dir, btn, facing, frame)
	local bit0 = (dir % 2) >= 1
	local bit1 = (math.floor(dir / 2) % 2) >= 1
	local _left, _right
	if facing == 0 then _left, _right = bit1, bit0 else _left, _right = bit0, bit1 end
	if _left  then frame[1] = true end
	if _right then frame[2] = true end
	if (math.floor(dir / 8) % 2) >= 1 then frame[3] = true end
	if (math.floor(dir / 4) % 2) >= 1 then frame[4] = true end
	for _b, _row in pairs(memory_button_row) do
		if (math.floor(btn / 2 ^ _b) % 2) >= 1 then frame[_row] = true end
	end
end
...
-- THE EMERGENCY REFILL IS ONE BAT, ON PURPOSE. DO NOT "FIX" IT TO MAX LIFE.
--
-- 144 is one bat: the game starts a round with what is in $FF815A, filled from
-- the table at ROM 0x0092B6 (144 / 288 / 432 / 576) by the cabinet's VITALITY
-- dip, and the standard setting is 288 - two bats.
--
-- Writing 288 here looks like the obvious correction, and it was made and
-- withdrawn: RESTORING 288 DOES NOT BRING THE SECOND BAT BACK on the gauge
-- (user, 2026-09-14). The character is left holding life the gauge does not
-- show, which reads as a bug rather than a refill. One bat is a state the
-- display can express, so one bat is what goes in.
--
-- The open question is what else has to be written for the second bat to come
-- back. Nobody has found it yet; until then this stays at one bat.
--
-- p1life runs every drawn frame out of runCheats; p2life is left called from
-- nowhere, the way it was.
local ONE_BAT = 0x0090

local function refill_to_one_bat(base)
	if memory.readword(base + 0x52) <= 28 then
		memory.writeword(base + 0x50, ONE_BAT)
		memory.writeword(base + 0x52, ONE_BAT)
	end
end

function p1life()
	refill_to_one_bat(0xFF8400)
end

function p2life()
	refill_to_one_bat(0xFF8800)
end
...
vsavScriptModule = {
    ["registerAfter"] = function()
		...
		-- P1 still comes from the pad: it is a human on a controller and the
		-- once-per-frame sample is what they are pressing.
		thisframe = {}
		filterinput(1, thisframe)
		compositeinput(thisframe)
		detectchanges(lastframe[1], thisframe)
		updaterecords(1, thisframe, inp[1])
		lastframe[1] = thisframe

		local _q = globals and globals.p2_tick_inputs
		if _q ~= nil and #_q > 0 then
			for _i = 1, #_q do
				thisframe = {}
				memoryinput(_q[_i].dir, _q[_i].btn, _q[_i].facing, thisframe)
				compositeinput(thisframe)
				detectchanges(lastframe[2], thisframe)
				updaterecords(2, thisframe, inp[2])
				lastframe[2] = thisframe
			end
			globals.p2_tick_inputs = {}
		else
			-- Nothing changed on any tick this frame. Age the idle counter so
			-- old lines still clear after `timeout`, without adding a record.
			changed = false
			updaterecords(2, lastframe[2] or {}, inp[2])
		end
	end,
	...
	["registerLoad"] = function(slot)
		-- loadscriptdata wants a NUMBERED slot. The tool's own state is a named
		-- one ("current_recording", from Use Savestate Upon Recording), and
		-- loading it hands this callback that name instead of a number, which
		-- raised "number expected, got string" and took the script down. Fall
		-- through to the defaults below in that case.
		if type(slot) == "number" then
			draw, inp, idle = savestate.loadscriptdata(slot)
		else
			draw, inp, idle = nil, nil, nil
		end
	...
	["guiRegister"] = function()
		...
		--
		-- It used to be drawn inside the display_hud branch, so Show P2 Inputs
		-- did nothing unless the life and meter readout was also on - a pairing
		-- with no reason behind it beyond where the code happened to sit, and one
		-- nobody could guess from the menu (user, 2026-09-13).
		--
		-- draw[2] stays: it is a local that travels in the savestate script data
		-- (see registerStart above), and is never written to anything but true.
		-- The option is the switch.
		if draw[2] and globals.options.display_p2_inputs ~= false then
