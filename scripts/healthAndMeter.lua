...
local healthAndMeter = {
    ["registerBefore"] = function()
        ...
        -- Dark Force
        --
        -- TWO ROWS, TWO IFS. This was an if/elseif, so P2's switch did nothing
        -- at all while P1's was on - and both shipped ON, which is the state a
        -- fresh install started in. They are independent settings and one
        -- cannot stand in for the other (2026-09-14).
        if globals.options.p1_infinite_df then
            memory.writeword(0xFF8400 + 0x176, 0x60)
        end
        if globals.options.p2_infinite_df then
