  ...
  function read_player_vars(_player1_obj, _player2_obj)
    ...
    if last_dummy_dict then
      ...
      if current and prev then
        ...
        -- KNOWN BUG, DEFERRED 2026-08-20.
        --
        -- jump_in_attack_start is never cleared once a reading is taken, so a
        -- landing that hit NOTHING is still measured against the previous
        -- jump-in's start. The diff < 30 guard hides it most of the time, but
        -- jump again quickly and a bogus number is recorded as if it were a
        -- jump-in. The fix is to nil the start here after using it, so nothing
        -- is measured until the next airborne hit sets it again.
        --
        -- Two more things are wrong with this trainer while it is being
        -- touched: p2_is_blocking_or_hit ($8805 == 2) is BLOCK as well as hit,
        -- so the menu's "p2 getting hit" is not what is measured; and both
        -- edges are taken with emu.framecount(), i.e. DISPLAYED frames.
        -- rawStateService.lua already has the tick/frame truth (ticker,
        -- framer, get_frame_from_tick) that playerStateService.lua uses for
        -- the dash-attack readout - that is what this should be built on
        -- rather than a second tick clock.
        ...
        -- The gap between recovering from hit or block stun and the next one
        -- used to be measured here with emu.framecount(). That counts DISPLAYED
        -- frames, about 1.6 ticks each, so the printed number was neither the
        -- game's count nor a stable one. guardCancel.lua's tick hook now fills
        -- globals.frames_between_attacks from the same $8805 == 2 edge, per
        -- tick, so the reading is in the unit the game actually uses.
